class BaseAccessory {
    ...
    convertColorTemperatureFromHomeKitToTuya(value) {
        const min = this.device.context.minWhiteColor || 0;
        const max = this.device.context.maxWhiteColor || 1000;
        const mink = this.device.context.minKelvin || 2700;
        const maxk = this.device.context.maxKelvin || 6500;
        
        const kelvin = 1000000 / value;
        const tuya = Math.round(Math.min(max, Math.max(min, (((kelvin - mink) / (maxk - mink)) * (max - min)) + min)));
    ...
    convertColorTemperatureFromTuyaToHomeKit(value) {
        ...
        const kelvin = Math.min(maxk, Math.max(mink, (((value - min) / (max - min)) * (maxk - mink)) + mink));
