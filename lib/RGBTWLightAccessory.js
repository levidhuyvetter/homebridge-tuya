...
class RGBTWLightAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        const characteristicColorTemperature = service.getCharacteristic(Characteristic.ColorTemperature)
            .setProps({
                minValue: Math.round(1000000 / (this.device.context.maxKelvin || 6500)), //use the actual kelvin range of the light converted to mireds if set in config or default to 2700-6500 which seems to be the range of most lights for sale
                maxValue: Math.round(1000000 / (this.device.context.minKelvin || 2700))
            })
